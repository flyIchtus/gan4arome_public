...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 28 13:48:51 2022
...
trainer_horovd version 3

    Include :
        
        -> split methods for readability
        -> add schedulers handling
        -> improve load/save logic
...
class Trainer():
    ...
    bound methods :
        ...
        logging methods :
            ...
            save_all : save models, optimizers, schedulers
    ...
    def instantiate_optimizers(self, modelG, modelD, load_optim, load_sched):
        ...
        if self.LA_optimizer :
                ...
                self.optim_G=LA.LookAheadOpt(self.optim_G)
                self.optim_D=LA.LookAheadOpt(self.optim_D)
    ...
    def save_all(self,Step,modelG,modelD):
        print("Saving")
    ...
    def test_(self, modelG, modelD, Step, DataIter):
        ...
        if hvd.rank()==0:
            ...
            if CRIT.item()==min(self.crit_List): 
                ...
                self.save_all(Step, modelG, modelD)
    ...
    def fit_(self, modelG, modelD):
        ...
        for e in range(self.config.epochs_num):
            ...
            while step<N_batch:
                ...
                if self.save_step>0 and Step%self.save_step==0 and hvd.rank()==0:
                    self.save_all(Step, modelG, modelD)
