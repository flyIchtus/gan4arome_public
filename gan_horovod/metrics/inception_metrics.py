...
def loadInception_v3(path, cuda=False, rank=0):
    """
    Instantiate a pretrained version of inception v3 whose outputs are Pool3 feature maps
    
    weights downloaded from https://download.pytorch.org/models/inception_v3_google-0cc3c7bd.pth
...
class InceptionV3(nn.Module):
    ...
    def __init__(self,
                 ...
                 resize_input=True, #specifying True may increase inception performance at the price of extra cost
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-3, module=np):
    ...
    return (module.linalg.norm(diff)**2 + module.trace(sigma1) +
...
class FIDclass():
    ...
    def FID(self, real_samples, fake_samples):
        ...
        mu1, sigma1=activation_statistics(real_samples, MODEL)
        mu2, sigma2=activation_statistics(fake_samples, MODEL)
